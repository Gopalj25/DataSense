...
const COLOUR_SWATCHES = ['#818CF8', '#34D399', '#FB923C', '#F472B6', '#38BDF8', '#A78BFA', '#FBBF24', '#F87171'];
...
export function autoSuggestAxes(type: string, columnMeta: ColumnMeta): Partial<EditorConfig> {
    ...
    switch (type) {
        case 'Bar Chart': case 'Line Chart':
            return { x_key: categoricalCols[0] ?? allCols[0], y_keys: numericCols.slice(0, 1) };
        case 'Pie Chart': return { label_key: categoricalCols[0] ?? allCols[0], value_key: numericCols[0] };
        case 'Scatter Plot': return { x_key: numericCols[0], y_keys: [numericCols[1] ?? numericCols[0]] };
        case 'Histogram': return { x_key: numericCols[0] };
        case 'Box Plot': return { x_key: categoricalCols[0] ?? allCols[0], y_keys: [numericCols[0]] };
        case 'Heatmap': return { columns: numericCols.slice(0, 10) };
        default: return {};
    }
}
...
export function validateConfig(config: EditorConfig, columnMeta: ColumnMeta): string | null {
    ...
    if (config.type === 'Pie Chart') {
        if (!config.label_key || !cols.includes(config.label_key)) return 'Select a label column.';
...
function TypePicker({ value, onChange }: { value: string; onChange: (t: string) => void }) {
    return (
        <div>
            <p style={{ fontSize: 10, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 6 }}>Chart Type</p>
            <div className="flex flex-wrap gap-1.5" role="group">
                {CHART_TYPES.map(ct => {
                    const Icon = ct.icon;
                    const active = ct.id === value;
                    return (
                        <button key={ct.id} onClick={() => onChange(ct.id)} aria-pressed={active}
...
function AxisSelect({ label, value, options, onChange, badge }: {
    ...
    return (
        <div>
            <p style={{ fontSize: 10, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 4 }}>{label}</p>
            <select value={value || ''} onChange={e => onChange(e.target.value)}
                ...
                {options.map(col => <option key={col} value={col}>{col} {badge ? `(${badge(col)})` : ''}</option>)}
            </select>
        </div>
    );
}
...
function ColourPicker({ value, onChange }: { value: string; onChange: (hex: string) => void }) {
    return (
        <div>
            <p style={{ fontSize: 10, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 6 }}>Primary Colour</p>
            <div className="flex gap-2">
                {COLOUR_SWATCHES.map(hex => (
                    <button key={hex} onClick={() => onChange(hex)}
                        ...
                        style={{ backgroundColor: hex, ringColor: 'var(--accent)', ringOffsetColor: 'var(--bg-surface)' }} title={hex} />
                ))}
            </div>
        </div>
    );
...
export default function ChartEditor({ fileId, aiConfig, columnMeta, currentPlotlyJson, onApply }: ChartEditorProps) {
    ...
    const patch = useCallback((p: Partial<EditorConfig>) => { setCfg(prev => ({ ...prev, ...p })); setHasChanges(true); setError(null); }, []);
    const handleTypeChange = useCallback((newType: string) => { patch({ type: newType, ...autoSuggestAxes(newType, columnMeta) }); }, [columnMeta, patch]);

    const addYKey = () => { const unused = numericColumns.filter(c => !cfg.y_keys?.includes(c)); if (unused.length) patch({ y_keys: [...(cfg.y_keys || []), unused[0]] }); };
    const removeYKey = (idx: number) => { const next = [...(cfg.y_keys || [])]; next.splice(idx, 1); patch({ y_keys: next }); };
    const setYKey = (idx: number, val: string) => { const next = [...(cfg.y_keys || [])]; next[idx] = val; patch({ y_keys: next }); };
    ...
    const handleApply = async () => {
        ...
        try {
            const res = await axios.post('http://localhost:8000/api/render', {
                file_id: fileId, chart_type: cfg.type, title: cfg.title, x_key: cfg.x_key, y_keys: cfg.y_keys,
                label_key: cfg.label_key, value_key: cfg.value_key, tooltip_key: cfg.tooltip_key, nbins: cfg.nbins, columns: cfg.columns, color: cfg.color,
            });
            onApply(res.data.plotly_json, cfg); setHasChanges(false);
        } catch (e: any) { setError(e.response?.data?.detail || 'Render failed.'); }
        finally { setIsPending(false); }
    };

    const handleReset = async () => {
        setCfg({ ...aiConfig, color: aiConfig.color || COLOUR_SWATCHES[0] }); setError(null); setHasChanges(false);
        setIsPending(true);
        try {
            const res = await axios.post('http://localhost:8000/api/render', {
                file_id: fileId, chart_type: aiConfig.type, title: aiConfig.title, x_key: aiConfig.x_key, y_keys: aiConfig.y_keys,
                label_key: aiConfig.label_key, value_key: aiConfig.value_key, tooltip_key: aiConfig.tooltip_key,
            });
            onApply(res.data.plotly_json, aiConfig);
        } catch { if (currentPlotlyJson) onApply(currentPlotlyJson, aiConfig); }
        finally { setIsPending(false); }
    };
        ...
        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
            ...
            <div className="space-y-4" style={{ padding: '16px 20px' }}>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <input type="text" value={cfg.title} onChange={e => patch({ title: e.target.value })}
                        className="flex-1 rounded-lg text-sm font-semibold outline-none transition"
                        style={{ background: 'var(--bg-inset)', border: '1px solid var(--border)', padding: '6px 12px', color: 'var(--text-primary)' }}
                        placeholder="Chart title…" />
                    {hasChanges && (
                        <span style={{ fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 20, background: 'var(--warning-dim)', color: 'var(--warning)', border: '1px solid rgba(251,191,36,0.2)' }}>Unsaved</span>
                    )}
                    ...
                    <button onClick={handleApply} disabled={isPending}
                        ...
                        {isPending ? <span className="w-3 h-3 border-2 border-white/40 border-t-white rounded-full animate-spin" /> : <Check size={12} />}
                        Apply
                    </button>
                </div>
                    ...
                    <div className="space-y-2">
                        {showXY && (
                            <>
                                ...
                                <div>
                                    <p style={{ fontSize: 10, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 4 }}>Y Axis (series)</p>
                                    {(cfg.y_keys || []).map((yk, i) => (
                                        <div key={i} className="flex gap-1 mb-1">
                                            <select value={yk} onChange={e => setYKey(i, e.target.value)}
                                                className="flex-1 rounded-lg text-sm outline-none transition"
                                                style={{ background: 'var(--bg-inset)', border: '1px solid var(--border)', padding: '6px 10px', color: 'var(--text-primary)' }}>
                                                {yOptions.map(c => <option key={c} value={c}>{c}</option>)}
                                            </select>
                                            {(cfg.y_keys?.length || 0) > 1 && <button onClick={() => removeYKey(i)} style={{ color: 'var(--danger)' }}><Trash2 size={14} /></button>}
                                        </div>
                                    ))}
                                    {(cfg.y_keys?.length || 0) < 3 && numericColumns.length > (cfg.y_keys?.length || 0) && (
                                        <button onClick={addYKey} className="flex items-center gap-1 text-xs mt-0.5" style={{ color: 'var(--accent-text)' }}>
                        ...
                        {showHist && (
                            <>
                                ...
                                <div>
                                    <p style={{ fontSize: 10, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 4 }}>Bins</p>
                                    <input type="number" min={5} max={200} value={cfg.nbins || 30} onChange={e => patch({ nbins: parseInt(e.target.value) || 30 })}
